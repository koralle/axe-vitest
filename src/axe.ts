...
const mount = (html: Element | string): [HTMLElement, () => void] => {
  if (isHTMLElement(html)) {
    ...
    return mountHTMLString(html.outerHTML)
  }

  if (isHTMLString(html)) {
    return mountHTMLString(html)
  }
...
const mountHTMLString = (htmlString: string): [HTMLElement, () => void] => {
...
function configureAxe(
   ...
   */
  return function axe(html: Element | string, additionalOptions: AxeCore.RunOptions = {}): Promise<AxeCore.AxeResults> {
