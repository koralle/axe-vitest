 ...
 */
export const toHaveNoViolations = (results: AxeCore.AxeResults): NoViolationsMatcherResult => {
...
const filterViolations = (violations: AxeCore.Result[], impactLevels: AxeCore.ImpactValue[]): AxeCore.Result[] => {
  if (impactLevels?.length > 0) {
    return violations.filter((v) => v.impact && impactLevels.includes(v.impact))
  }
