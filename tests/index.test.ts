...
import { configureAxe, axe } from '../src'
...
describe('axe', () => {
  ...
  test('throws if input is not a string, vue element, react element, or react testing library render', async () => {
    // @ts-ignore
    await expect(axe({})).rejects.toThrow('html parameter should be an HTML string or an HTML element')
  })

  test('throws with non-html input', async () => {
    await expect(axe('Hello, World')).rejects.toThrow('html parameter ("Hello, World") has no elements')
  })
